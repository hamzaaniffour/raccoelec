          ...
          {item.childItems.nodes.map((childItem: MenuItem) => (
            <li key={childItem.id}>
              <Link
                ...
                className={`block text-slate-600 hover:text-slate-800 text-md py-2 px-3 border-b-[1px] border-slate-200 font-medium`}
              >
                {childItem.label}
              </Link>
            </li>
...
export default async function Menu() {
  const menuItems = await getMenuByName("bootstrap-menu");
